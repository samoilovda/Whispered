...
> **Статус (2026-08-17): `export_controller` сделан; `transcription_controller`
> и `pipeline_controller` сознательно не начаты.** См. обоснование в разделе 9
> — вынесение в отдельный файл без реального сокращения Qt-связности не
> проходит правило "не добавлять абстракцию сверх необходимого".
...
> Шаг 3 — закрыт полностью: Cover, article, YouTube, book и (после
> добавления собственно экспорта, по отдельному запросу владельца —
> см. ниже) insights.** `ui/cover_view.py::_write_provenance` пишет
...
>
> **`InsightsPanel` изначально не имела файловой записи вообще** —
> `_render_chapters`/`_render_action_items`/`_render_key_moments` только
> рисовали виджеты и не сохраняли сырые данные. Это было за пределами
> R5-full (миграция существующего пути ≠ новая фича) до отдельного
> запроса владельца добавить экспорт. По запросу добавлено:
> `core/insights_export.py` — Qt-free форматтеры (по образцу
> `core/youtube_description.py`), панель хранит `self._results` (раньше
> отбрасывалось сразу после рендера), кнопка "Save to file" рядом с
> "Generate" пишет каждую непустую секцию в `output_dir()` — один `.txt`
> на секцию — с той же best-effort Artifact-манифест записью, что и
> остальные четыре генератора. В отличие от YouTube (одна видимая
> вкладка → одна кнопка "Save" для неё), Insights показывает все три
> секции сразу, так что "Save" естественно означает "сохранить все
> сгенерированные секции", а не "текущую вкладку" — единственная
> содержательная адаптация паттерна YouTube-панели.
...
> через рестарт), retry-with-backoff, и полный шаг 3 — миграция preset
> chain/YouTube/Insights/batch на сам движок `JobEngine.run()`.
>
> **Двойной расчёт `chapters` — закрыт отдельным, более узким путём, не
> через `JobEngine.run()`.** YouTube и Insights триггерятся в РАЗНОЕ
> время (`MainWindow._start_next_extra_chain_step`: Insights стартует
> только после того, как youtube+article уже завершились), а не одним
> батчем — это плохо ложится на модель "один `run()` на один `JobSpec`"
> без изменения порядка триггеров (реальное, непроверяемое из этого
> окружения продуктовое поведение). Вместо этого — `core/insights_cache.py`
> (`InsightsCache`), общий инстанс на `MainWindow`, передан в
> `YouTubePanel`/`InsightsPanel`/`CoverView`; `InsightsWorker` получил
> опциональный `cache=` параметр (по умолчанию `None`, поведение не
> меняется для существующих вызывающих) и проверяет/пишет в кэш по ключу
> `insight_type + built prompt + provider`. Это ровно тот механизм,
> который план называет причиной устранения дубля ("устраняется как
> следствие общего кэша"), просто не обёрнутый в `JobEngine.run()`.
> Тесты в `tests/test_insights_cache.py` доказывают дедупликацию
> счётчиком реальных LLM-вызовов.
...
(✅ закрыто через `InsightsCache`, не через сам `JobEngine.run()` — см.
статус выше); после принудительного завершения приложения `resume`
продолжает job с незавершённого шага (не проверено — нет persisted state);
суммарное число живых QThread не уменьшилось (генераторы не переведены на
движок).
...
- [2026-08-17] [R6-cont] `export_controller` оказался хорошим кандидатом на
  извлечение, потому что у `_export_result` был настоящий "вычислить
  результат, затем показать" шов: одиночный/множественный формат, success/
  failure aggregation — чистая логика, которую раньше нельзя было
  протестировать без полного Qt-окна. `transcription_controller` и
  `pipeline_controller` из плана — **сознательно не сделаны** после чтения
  кода: `_start_transcription`/`_on_progress`/`_reset_ui` — это почти
  целиком прямые вызовы виджетов (progress bar, кнопки, читение combo box),
  без скрытой бизнес-логики; то немногое, что было чистой логикой
  (`timeline_stage_for_progress`, `localized_progress`, `format_eta` для
  `_on_progress`), уже вынесено в `ui/transcription_progress.py` — вынести
  туда ещё нечего. Файл-переезд Qt-кода в `application/*_controller.py`
  без реального снижения Qt-связности не даёт тестируемости и не снижает
  риск — это ровно тот вид преждевременной абстракции, который
  запрещён project-правилами ("не добавлять абстракции сверх
  необходимого"). Оркестрация preset chain/extra-chain — то, что
